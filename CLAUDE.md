...
6. **`clustering::find_duplicate_groups`** — connected components via a disjoint set. A group asserts "a chain of matches runs through these", not "these are pairwise copies". Adds no thresholds of its own.
7. **`export::output_results`** — resolves KEEP/DELETE/REVIEW entirely inside one group (safe, because components partition the files), performs the disposal, and returns the paths that moved so `main` can drop their cache entries.
